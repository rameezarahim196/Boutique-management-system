...
void Salesman:: addEmployee(const Employee& e1) {
    ifstream file("salesman.json");
    json existingData;
    if (file.is_open()) {
        file >> existingData;
        file.close();
    ...
    ofstream outFile("salesman.json",ios::out);
    if (!outFile.is_open()) {
...
void Salesman::removeEmployeeFromJSONArray(const string& employeeNameToRemove) {
    ifstream file("salesman.json");
    json existingData;
    if (file.is_open()) {
        file >> existingData;
    ...
    ofstream outFile("salesman.json");
    if (outFile.is_open()) {
...
void Salesman::updateEmployeeInJSONArray(const Employee& updatedEmployee) {
    ifstream file("salesman.json");
    json existingData;
    if (file.is_open()) {
        file >> existingData;
