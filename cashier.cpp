...
Cashier::Cashier(int id, const string& name, const string& section, int age): Employee(id, name, section, age)
{}

void Cashier:: addEmployee(const Employee& e1) {
    ifstream file("cashier.json");
    json existingData;
    if (file.is_open()) {
        file >> existingData;
        file.close();
    ...
    ofstream outFile("cashier.json",ios::out);
    if (!outFile.is_open()) {
...
void Cashier::removeEmployeeFromJSONArray(const string& employeeNameToRemove) {
    ifstream file("cashier.json");
    json existingData;
    if (file.is_open()) {
        file >> existingData;
...
void Cashier::updateEmployeeInJSONArray(const Employee& updatedEmployee) {
    ifstream file("cashier.json");
    json existingData;
    if (file.is_open()) {
        file >> existingData;
